...
if (isset($_POST['submit'])){
            ...
            $query1=$con->query
            ("SELECT DATE_FORMAT(prod_date, '%Y%m%d') AS date1, count(*) as total FROM vaccine 
				WHERE vaccine.inoculatorSSN IS not null AND DATE(prod_date) <= '".$_POST['day0']."'");//입력받은 날짜 기준으로 누적 확진자수(확진자tbl에서 입원일을 가져와야함)
...
if (isset($_POST['submit'])){
            ...
            //$oquery=$con->query("select * from date WHERE day='".$_POST['day0']."'");
            $query2=$con->query
            ("SELECT DATE_FORMAT(prod_date, '%Y%m%d') AS date2, count(*) as waste_total FROM vaccine 
            WHERE vaccine.disuse IS NULL AND DATE(prod_date) <= '".$_POST['day0']."'");//입력받은 날짜 기준으로 누적 사망자수(확진자tbl에서 사망일을 가져와야함)
